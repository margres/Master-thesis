...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec 17 16:17:18 2020
...
def FtSingularFunc(images_info, tau_list):
    ...
    for index, row in images_info.iterrows():
        ...
        # min 
        if row['typeI'] == 'min':
            tmp[tau_list>=row['tauI']] = 1.*np.pi*(row['muI']**0.5)
            # print(">>>> a min image")
        # max 
        elif row['typeI'] == 'max':
            tmp[tau_list>=row['tauI']] = -1.*np.pi*(row['muI']**0.5)
        ...
        # saddle 
            ...
            tmp = -1.*((-row['muI'])**0.5)*np.log(np.absolute(tau_list-row['tauI']))
...
################ Plot F(t) extrapolated at high t   ########################
    ...
    mask_asymptote=t>np.max(tauI) #Ft_new>muI[np.argmax(tauI)] 
    geom_optics=muI[np.argmax(tauI)]**(1/2) / np.log(np.max(tauI)) 
    asymptote=np.ones(len(Ft_new[~mask_asymptote]))
    Ft_new = Ft_new[mask_asymptote]  
    Ft_new = np.concatenate([Ft_new,asymptote])
    
    '''
    At high t it goes back to the geometrical optics. which
    '''
